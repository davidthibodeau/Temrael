...
namespace Server.Mobiles
{
	...
	public class CuSidhe : BaseMount
	{
		...
		[Constructable]
		public CuSidhe( string name ) : base( name, 400, 0x3E91, AIType.AI_Animal, FightMode.Aggressor, 10, 1, 0.2, 0.4 )
		{			
			...
			SetResistance( ResistanceType.Magie, 70, 85 );

			SetSkill( SkillName.Anatomie, 90.1, 96.8 );
			SetSkill( SkillName.Tactiques, 90.3, 99.3 );
			SetSkill( SkillName.Concentration, 75.3, 90.0 );
			//SetSkill( SkillName.Anatomy, 65.5, 69.4 );
			SetSkill( SkillName.Soins, 72.2, 98.9 );
		...
		{
			if ( from == ControlMaster && from.AccessLevel == AccessLevel.Player )
			{
