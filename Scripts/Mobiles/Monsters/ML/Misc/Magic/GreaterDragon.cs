...
namespace Server.Mobiles
{
	...
	public class GreaterDragon : BaseCreature
	{
		...
		public GreaterDragon () : base( AIType.AI_Mage, FightMode.Closest, 10, 1, 0.3, 0.5 )
		{
			...
			SetResistance( ResistanceType.Contondant, 65, 90 );
			SetResistance( ResistanceType.Tranchant, 40, 55 );
			SetResistance( ResistanceType.Perforant, 40, 60 );
			SetResistance( ResistanceType.Magie, 50, 75 );

			SetSkill( SkillName.Concentration, 0 );
			//SetSkill( SkillName.EvalInt, 110.0, 140.0 );
			SetSkill( SkillName.ArtMagique, 110.0, 140.0 );
			SetSkill( SkillName.Empoisonnement, 0 );
			//SetSkill( SkillName.Anatomy, 0 );
			SetSkill( SkillName.Concentration, 110.0, 140.0 );
			SetSkill( SkillName.Tactiques, 110.0, 140.0 );
			SetSkill( SkillName.Anatomie, 115.0, 145.0 );
		...
		public override void Deserialize( GenericReader reader )
		{
			...
			if( version == 0 )
			{
				...
				Skills[SkillName.ArtMagique].Base = Skills[SkillName.ArtMagique].Cap; // Greater dragons have a 90% cap reduction and 90% skill reduction on magery
			}
		}
	}
}
