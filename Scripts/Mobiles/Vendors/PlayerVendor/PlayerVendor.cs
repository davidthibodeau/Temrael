	...
	public class PlayerVendor : Townsperson
	{
		...
		public override bool OnDragDrop( Mobile from, Item item )
		{
			...
			if ( item is Gold )
			{
				if ( BaseHouse.NewVendorSystem )
				{
					...
					{
                        SayTo(from,"Je prendrai ceci pour payer mon salaire.");
			...
			else
			{
				...
				{
                    SayTo(from, "Je ne peux vendre plus de marchandise.");
		...
		public override bool CheckNonlocalLift( Mobile from, Item item )
		{
			if ( item.IsChildOf( this.Backpack ) )
			{
				...
				{
                    SayTo(from, "Si vous souhaitez acheter un objet faites moi signe.");
		...
		public void OpenBackpack( Mobile from )
		{
			if ( this.Backpack != null )
			{
                SayTo(from, "Voici ce que je vends.");
		...
		public static void TryToBuy( Item item, Mobile from )
		{
			...
			if ( vi == null )
			{
                vendor.SayTo(from, "Vous ne pouvez pas acheter ceci.");
				//vendor.SayTo( from, 503216 ); // You can't buy that.
			}
			else if ( !vi.IsForSale )
			{
                vendor.SayTo(from, "Cet objet n'est pas en vente.");
		...
		public void CollectGold( Mobile to )
		{
			...
			{
                SayTo(to, "Je n'ai pas d'or pour vous en ce moment.");
		...
		public void Rename( Mobile from )
		{
            ...
            from.SendMessage("Entrez le nom de votre vendeur.");
		...
		public override void GetContextMenuEntries( Mobile from, List<ContextMenuEntry> list )
		{
            ...
            list.Add(new Server.Mobiles.PlayerMobile.CallbackEntry(6096, new Server.Mobiles.PlayerMobile.ContextCallback(OpenSac)));
            list.Add(new Server.Mobiles.PlayerMobile.CallbackEntry(6103, new Server.Mobiles.PlayerMobile.ContextCallback(BuyFromInventory)));
            if (IsOwner(from))
                list.Add(new Server.Mobiles.PlayerMobile.CallbackEntry(6095, new Server.Mobiles.PlayerMobile.ContextCallback(OpenCustomizationMenu)));
        }

        private void OpenSac()
        {
            if (m_Mobile != null)
            {
                m_Mobile.SendMessage("Acheter un objet de l'inventaire en choisissant 'Acheter' du menu du NPC.");
                this.OpenBackpack(m_Mobile);
            }
        }

        private void BuyFromInventory()
        {
            if (m_Mobile != null)
            {
                m_Mobile.SendMessage("Choisissez un objet de l'inventaire (le sac) du vendeur pour acheter.");
                m_Mobile.Target = new PVBuyTarget();
            }
        }
        ...
        private void OpenCustomizationMenu()
        {
            if (m_Mobile != null)
            {
                m_Mobile.SendGump(new PlayerVendorOwnerGump(this));
            }
        }
		...
		private class VendorNamePrompt : Prompt
		{
			...
			public override void OnResponse( Mobile from, string text )
			{
				...
				{
					m_Vendor.SayTo( from, "Le nom n'est pas acceptable." );
					return;
				}
