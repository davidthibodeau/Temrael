...
namespace Server.Mobiles
{
    ...
    public class Townsperson : BaseCreature
    {
        ...
        private bool m_SpeechSalutation = false;
        private bool m_SpeechAurevoir = false;
        private bool m_SpeechOccupation = false;
        private bool m_SpeechPresentation = false;
        private bool m_SpeechTemperature = false;
        ...
        public override void GetContextMenuEntries(Mobile from, List<ContextMenuEntry> list)
        {
            ...
            if (m_SpeechSalutation)
                list.Add(new SalutationEntry(from, this));

            if (m_SpeechAurevoir)
                list.Add(new AurevoirEntry(from, this));

            if (m_SpeechOccupation)
                list.Add(new OccupationEntry(from, this));

            if (m_SpeechPresentation)
                list.Add(new PresentationEntry(from, this));

            if (m_SpeechTemperature)
                list.Add(new TempsEntry(from, this));

            if (m_SpeechRumeur)
                list.Add(new RumeurEntry(from, this));

            if (m_SpeechFamille)
                list.Add(new FamilleEntry(from, this));
        }

        private class SalutationEntry : ContextMenuEntry
        {
            private Mobile m_From;
            private Townsperson m_NPC;

            public SalutationEntry(Mobile from, Townsperson person)
                : base(6250, -1)
            {
                m_From = (Mobile)from;
                m_NPC = (Townsperson)person;
            }

            public override void OnClick()
            {
                Response response = new Response();
                SpeechResponse ret = response.GetResponse("salutations", m_From, m_NPC);
                m_NPC.SpeechHandler(ret);
            }
        }

        private class AurevoirEntry : ContextMenuEntry
        {
            private Mobile m_From;
            private Townsperson m_NPC;

            public AurevoirEntry(Mobile from, Townsperson person)
                : base(6251, -1)
            {
                m_From = (Mobile)from;
                m_NPC = (Townsperson)person;
            ...
            {
                Response response = new Response();
                SpeechResponse ret = response.GetResponse("aurevoir", m_From, m_NPC);
                m_NPC.SpeechHandler(ret);
            }
        }

        private class PresentationEntry : ContextMenuEntry
        {
            private Mobile m_From;
            private Townsperson m_NPC;

            public PresentationEntry(Mobile from, Townsperson person)
                : base(6252, -1)
            {
                m_From = (Mobile)from;
                m_NPC = (Townsperson)person;
            ...
            {
                Response response = new Response();
                SpeechResponse ret = response.GetResponse(Utility.RandomBool() ? "mon nom" : "ton nom", m_From, m_NPC);
                m_NPC.SpeechHandler(ret);
            }
        }

        private class TempsEntry : ContextMenuEntry
        {
            private Mobile m_From;
            private Townsperson m_NPC;

            public TempsEntry(Mobile from, Townsperson person)
                : base(6253, -1)
            {
                m_From = (Mobile)from;
                m_NPC = (Townsperson)person;
            ...
            {
                Response response = new Response();
                SpeechResponse ret = response.GetResponse("le temps", m_From, m_NPC);
                m_NPC.SpeechHandler(ret);
            }
        }

        private class RumeurEntry : ContextMenuEntry
        {
            private Mobile m_From;
            private Townsperson m_NPC;

            public RumeurEntry(Mobile from, Townsperson person)
                : base(6254, -1)
            {
                m_From = (Mobile)from;
                m_NPC = (Townsperson)person;
            ...
            {
                Response response = new Response();
                SpeechResponse ret = response.GetResponse("potin", m_From, m_NPC);
                m_NPC.SpeechHandler(ret);
            }
        }

        private class OccupationEntry : ContextMenuEntry
        {
            private Mobile m_From;
            private Townsperson m_NPC;

            public OccupationEntry(Mobile from, Townsperson person)
                : base(6255, -1)
            {
                m_From = (Mobile)from;
                m_NPC = (Townsperson)person;
            ...
            {
                Response response = new Response();
                SpeechResponse ret = response.GetResponse("emploi", m_From, m_NPC);
                m_NPC.SpeechHandler(ret);
            }
        }

        private class FamilleEntry : ContextMenuEntry
        {
            private Mobile m_From;
            private Townsperson m_NPC;

            public FamilleEntry(Mobile from, Townsperson person)
                : base(6256, -1)
            {
                m_From = (Mobile)from;
                m_NPC = (Townsperson)person;
            ...
            {
                Response response = new Response();
                SpeechResponse ret = response.GetResponse("famille", m_From, m_NPC);
                m_NPC.SpeechHandler(ret);
            }
        }
		...
		{
			"*salut*", "*hoche*", "*souris*", "b'jour", 
			"*salut de la main*", "*hoche de la tete*", "*mince sourire*",
			"*salut*", "*opine de la tete*", "*smiles*",
		};
        private static string[] badGreetings = 
		{
			"*opine rapidement*", "*fronce des sourcils*", "*hoche faiblement*", "*fronce des sourcils*", 
			"*toux*", "*hrumph*", "*grogne*"
		};
        ...
        public virtual TimeSpan PauseDelay { get { return TimeSpan.FromSeconds(60); } }
        ...
        public Townsperson(string name, string title, string tag)
            : base(AIType.AI_Thief, FightMode.Closest, 10, 1, 0.8, 1.6)
        {
            Name = name;
            Title = title;
            Tag = tag;
        ...
        [Constructable]
        public Townsperson(AIType aiType, FightMode fightmode, int RangePerception, int RangeFight, double ActiveSpeed, double PassiveSpeed)
            : base(AIType.AI_Thief, FightMode.Closest, RangePerception, RangeFight, ActiveSpeed, PassiveSpeed)
        {
            //Name = name;
            //Title = title;
            //Tag = tag;
        ...
        public override void Serialize(GenericWriter writer)
        {
            ...
            writer.Write(m_SpeechOccupation);
            writer.Write(m_SpeechPresentation);
            writer.Write(m_SpeechTemperature);
            writer.Write(m_SpeechRumeur);
            writer.Write(m_SpeechFamille);
