	...
	class Meditation
	{
		...
		public static TimeSpan OnUse( Mobile m )
		{
            ...
            if (m is TMobile)
            {
                ...
                if (tmob.GetAptitudeValue(NAptitude.PortArmeMagique) >= 3)
			...
			else if (( Core.AOS && Server.Misc.RegenRates.GetArmorOffset( m ) > 0 ) && (!canMedidatePortArme))
			{
			...
			else 
			{
				...
				double skillVal = m.Skills[SkillName.Concentration].Value;
				double chance = (50.0 + (( skillVal - ( m.ManaMax - m.Mana ) ) * 2)) / 100;

				if ( chance > Utility.RandomDouble() )
				{
					m.CheckSkill( SkillName.Concentration, 0.0, 100.0 );
