...
namespace Server.SkillHandlers
{
	public class AnimalTaming
	{
		...
		{
			SkillInfo.Table[(int)SkillName.Dressage].Callback = new SkillUseCallback( OnUse );
		}
		...
		private class InternalTarget : Target
		{
			...
			private class InternalTimer : Timer
			{
				...
				protected override void OnTick()
				{
					...
					if ( !m_Tamer.InRange( m_Creature, 18 ) )
					{
					...
					else if ( m_Count < m_MaxCount )
					{
						...
						if ( !alreadyOwned ) // Passively check animal lore for gain
							m_Tamer.CheckTargetSkill( SkillName.Dressage, m_Creature, 0.0, 120.0 );
					...
					else
					{
						...
						if ( CheckMastery( m_Tamer, m_Creature ) || alreadyOwned || m_Tamer.CheckTargetSkill( SkillName.Dressage, m_Creature, minSkill - 25.0, minSkill + 25.0 ) )
						{
