    ...
    public abstract class BaseVendor : Townsperson, IVendor
	{
		...
		public virtual void VendorBuy( Mobile from )
		{
			...
			if ( list.Count > 0 )
			{
                ...
                SayTo(from, "Observez mes marchandises.");
				//SayTo( from, 500186 ); // Greetings.  Have a look around.

                //m_buyer = from;
                //Timer.DelayCall(TimeSpan.FromSeconds(1), new TimerCallback(SendMobileInfo));
			}
		}
        ...
        {
            if (m_buyer != null)
                m_buyer.Send(new MobileStatusExtended(m_buyer, false));
        }*/
		...
		{
            //buyer.Send(new MobileStatusExtended(buyer, false));
			...
			if ( !bought && cont != null )
			{
                if (cont.ConsumeTotal(typeof(Gold), totalCost))
                    ...
                    SayTo(buyer, "Vous ne pouvez pas payer pour cela.");
			...
			if ( !bought && totalCost >= 2000 )
			{
				...
				{
                    SayTo(buyer, "Vous n'avez pas de quoi payer.");
