	...
	public class SymphonieSpell : BardeSpell
	{
		...
		public override void OnCast()
        {
            if (CheckSequence())
            {
                ...
                //définition des cibles du sort selon la portée
                if (map != null)
                {
                    foreach (Mobile m in Caster.GetMobilesInRange(portee))
                    {
                        //if (Caster.CanBeBeneficial(m, false) && TMobile.IsInEquipe(Caster, m))
                ...
                for (int i = 0; i < m_target.Count; ++i)
                {
                    ...
                    m_SymphonieTable[targ] = factor;

                    Timer t = new SymphonieTimer(targ, DateTime.Now + duration);
                    m_Timers[targ] = t;
                    t.Start();

                    targ.FixedParticles(14186, 10, 20, 5013, 1932, 0, EffectLayer.Head); //ID, speed, dura, effect, hue, render, layer
                    targ.PlaySound(494);
                }
            }
        ...
        public void StopTimer(Mobile m)
        {
            ...
            if (t != null)
            {
                ...
                m.FixedParticles(14186, 10, 20, 5013, 1932, 0, EffectLayer.Head); //ID, speed, dura, effect, hue, render, layer
                m.PlaySound(494);
            }
        }
