...
namespace Server.Items
{
	public class SleeperBedBody : Container 
	{
		...
		public SleeperBedBody( Mobile owner, bool isSpell, bool AlwaysSleep, SleeperBaseAddon addon) : base( 0x2006 )
		{
			...
			m_Owner = owner;
			Name = m_Owner.Name;
			m_SleeperBedBodyName = "Dormeur/euse";
			Hue = m_Owner.Hue;
		...
		public override void OnDoubleClick( Mobile from )
		{
			...
			if (from == m_Owner) {
				if (m_AlwaysSleep) {
					from.SendMessage("Vous devez double cliquez le lit pour vous réveillez !");
				}
				else
				{
					m_Asleep = !m_Asleep;
					if (m_Asleep) {
						m_Timer = new InternalTimer( m_Owner, this );
						m_Timer.Start();
						this.PublicOverheadMessage(0,m_Owner.EmoteHue, false,"*Ferme les yeux.*");
						from.SendMessage( "Vous fermez vos yeux et commencer à dormir. (Double Cliquer le lit pour vous réveiller)" );
					}
					else
					{
						if (m_Timer != null)
							m_Timer.Stop();
						this.PublicOverheadMessage(0,m_Owner.EmoteHue, false,"*S'étire*");
						from.SendMessage( "Vous vous réveillez momentairement, mais êtes trop fatigué pour vous relevez. (Double cliquer le lit pour vous réveiller)" );
					}
				}
			}
			else
			{
				...
				if (m_AlwaysSleep) {
					from.LocalOverheadMessage( MessageType.Regular, from.SpeechHue, true, "Vous décidez de ne pas les réveiller." );
				}
				else
				{
					if (m_Asleep)
						from.LocalOverheadMessage( MessageType.Regular, from.SpeechHue, true, "Vous décidez de les laisser dormir." );
					else
						from.LocalOverheadMessage( MessageType.Regular, from.SpeechHue, true, "Ils sont déjà debout !" );
				}
			}
		}
		...
		public override void AddNameProperty( ObjectPropertyList list )
		{
			if ( m_SleeperBedBodyName != null )
				...
				list.Add( 1049644, String.Format( "{0} Qui se repose", Name ) );
		}
		...
		public override void OnSingleClick( Mobile from )
		{
			if (m_AlwaysSleep)
			{
				LabelTo( from, m_SleeperBedBodyName == null ? String.Format( "{0} Qui rêve", Name ) : m_SleeperBedBodyName );
			}
			else
			{
				if (m_Asleep)
					LabelTo( from, m_SleeperBedBodyName == null ? String.Format( "{0} Qui dort", Name ) : m_SleeperBedBodyName );
				else
					LabelTo( from, Name );
			}
		}
		...
		private class InternalTimer : Timer
		{
			...
			{ 
				...
				{
					Stop();
					m_Body.PublicOverheadMessage(0,m_Owner.EmoteHue, false,"*Tombe dans un sommeil profond.*");
				}
			} 
		}
	}
