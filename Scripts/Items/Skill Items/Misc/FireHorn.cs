...
namespace Server.Items
{
	public class FireHorn : Item
	{
		...
		public void Use( Mobile from, IPoint3D loc )
		{
			...
			int music = from.Skills[SkillName.Musique].Fixed;

			int sucChance = 500 + ( music - 775 ) * 2;
			double dSucChance = ((double)sucChance) / 1000.0;

			if ( !from.CheckSkill( SkillName.Musique, dSucChance ) )
			{
			...
			if ( targets.Count > 0 )
			{
				int prov = from.Skills[SkillName.Musique].Fixed;
				int disc = from.Skills[SkillName.Musique].Fixed;
                int peace = from.Skills[SkillName.Musique].Fixed;
				...
				for ( int i = 0; i < targets.Count; ++i )
				{
					...
					if ( !Core.AOS && m.CheckSkill( SkillName.Concentration, 0.0, 120.0 ) )
					{
						toDeal *= 0.5;
