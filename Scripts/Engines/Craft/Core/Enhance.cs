...
namespace Server.Engines.Craft
{
	...
	public class Enhance
	{
		public static EnhanceResult Invoke( Mobile from, CraftSystem craftSystem, BaseTool tool, Item item, CraftResource resource, Type resType, ref object resMessage )
		{
            ...
            bool allRequiredAptitudes = false;
			if( craftItem.GetSuccessChance( from, resType, craftSystem, false, ref allRequiredSkills, ref allRequiredAptitudes ) <= 0.0 )
			...
			bool contondantBonus = false;
			bool tranchantBonus = false;
			bool magieBonus = false;
			bool perforantBonus = false;
			...
			if ( item is BaseWeapon )
			{
				...
				magieBonus = ( attributes.WeaponMagieDamage > 0 );
				perforantBonus = ( attributes.WeaponPerforantDamage > 0 );
			...
			else
			{
				...
				tranchantBonus = ( attributes.ArmorTranchantResist > 0 );
				magieBonus = ( attributes.ArmorMagieResist > 0 );
				perforantBonus = ( attributes.ArmorPerforantResist > 0 );
