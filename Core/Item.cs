...
namespace Server
{
	...
	public class Item : IEntity, IHued, IComparable<Item>, ISerializable, ISpawnable
    {
        ...
        private class DecrireEntry : ContextMenuEntry
        {
            private Mobile m_From;
            private Item m_Item;

            public DecrireEntry(Mobile from, Item item)
                : base(6273, -1)
            {
                m_From = (Mobile)from;
                m_Item = (Item)item;
        ...
        private class UtiliserEntry : ContextMenuEntry
        {
            private Mobile m_From;
            private Item m_Item;

            public UtiliserEntry(Mobile from, Item item)
                : base(6272, -1)
            {
                m_From = (Mobile)from;
                m_Item = (Item)item;
            ...
            public override void OnClick()
            {
                m_Item.OnDoubleClick(m_From);
            }
        }

        private class LockdownEntry : ContextMenuEntry
        {
            private Mobile m_From;
            private Item m_Item;

            public LockdownEntry(Mobile from, Item item)
                : base(6290, -1)
            {
                m_From = (Mobile)from;
                m_Item = (Item)item;
        ...
        private class UnLockdownEntry : ContextMenuEntry
        {
            private Mobile m_From;
            private Item m_Item;

            public UnLockdownEntry(Mobile from, Item item)
                : base(6291, -1)
            {
                m_From = (Mobile)from;
                m_Item = (Item)item;
        ...
        public class DecrirePrompt : Prompt
        {
            private Item m_Item;
            private Mobile m_From;

            public DecrirePrompt(Mobile from, Item item)
            {
                from.SendMessage("Entrez le nouveau nom de l'objet souhaite :");
                m_Item = item;
                m_From = from;
            }
		...
		public virtual void GetContextMenuEntries( Mobile from, List<ContextMenuEntry> list )
		{
            ...
            if (m_Parent is Item)
            {
                list.Add(new UtiliserEntry(from, this));
                if (CanBeAltered)
                    list.Add(new DecrireEntry(from, this));
            }
            else if (m_Parent is Mobile)
            {
                if ((Mobile)m_Parent == from)
                {
                    list.Add(new UtiliserEntry(from, this));
                    if (CanBeAltered)
                        list.Add(new DecrireEntry(from, this));
            ...
            if ((m_Parent == null) && (Movable) && (CanBeAltered))
                list.Add(new LockdownEntry(from, this));
            else if ((m_Parent == null) && (CanBeAltered) && (!(Movable)))
                list.Add(new UnLockdownEntry(from, this));
		}
