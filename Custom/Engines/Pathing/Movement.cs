...
namespace Server.Movement
{
	public class MovementImpl : IMovementImpl
	{
		...
		private bool IsOk( bool ignoreDoors, int ourZ, int ourTop, StaticTile[] tiles, List<Item> items )
		{
		...
		public static bool CheckNoItemMob(Map map, Mobile m, int x, int y, int z)		
		{
			if( map != null )
			{
				...
				{
					Mobile mob = newSector.Mobiles[i];

					if( mob != m && mob.X == x && mob.Y == y && mob.Z < z +15 && mob.Z > z - 15)
						return false;
				}
				...
				for( int i = 0; i < newSector.Items.Count; ++i )
				{
					...
					if (item is BaseDoor && ((BaseCreature)m).CanOpenDoors) {}
					else if(item.ItemData.Impassable && item.AtWorldPoint( x, y ) && !item.Movable && (item.Z == z || ((item.Z + item.ItemData.Height) > z && (z + 15) > item.Z)))
						return false;
				}
			}
			return true;
		...
		public bool CheckMovement( Mobile m, Map map, Point3D loc, Direction d, out int newZ )
		{
			...
			// Stop people walking up mountains
			/*if (startZ +20 < newZ)
			{
				m.SendMessage("That is too walk to walk up");
				return false;
			}*/
